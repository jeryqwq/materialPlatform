...
function Preview(props: { fileSystem: FileSys }) {
  ...
  useLayoutEffect(() => {
    ...
    const options = {
      ...
      handleModule: async function (type: string, getContentData: Function, path: string, options: any) {  // 解析器优先使用handleModule
      ...
      getResource(pathCx: any, options: any) { // 其次使用getResource ， 当handleModule返回null或者不返回时， 源码位置 tools.ts:294
    ...
    Vue.createApp(Vue.defineAsyncComponent(() => _loader.loadModule('/index.vue', options))).mount(elWrap as unknown as HTMLElement)
    return () => {
